...
// --- GRAVAÇÃO COM VALIDAÇÃO DE CONFLITO (CORREÇÃO) ---
...
if(btnConfirm) btnConfirm.addEventListener('click', () => {
    ...
    const btn = document.getElementById('confirm-appointment-btn');
    btn.disabled = true;
    btn.textContent = "Verificando...";

    // Verificar se já existe consulta para este médico nesta data e hora
    db.collection("consultas")
        .where("profissionalId", "==", selectedProfessional.id)
        .where("data", "==", firebase.firestore.Timestamp.fromDate(finalDate))
        .where("status", "==", "Agendada") // Só conflita se estiver ativa
        .get()
        .then(snap => {
            if (!snap.empty) {
        ...
        .catch(err => {
            showCustomAlert("Atenção", err.message, "error");
            btn.disabled = false;
            btn.textContent = "Confirmar";
            // Opcional: Voltar para a tela de horário
            // showAppScreen(document.getElementById('datetime-screen'));
        });
});
